...
/**
 * This function enumerates all the .csproj files in the given directory (recursively) and updates
 * them by removing the Version attribute from all the PackageReference elements.
 * @param rootDir The root directory to start the search from.
 * @returns A promise that resolves when all the .csproj files have been updated.
 */
export async function updateCsprojFiles(rootDir: string): Promise<void> {
